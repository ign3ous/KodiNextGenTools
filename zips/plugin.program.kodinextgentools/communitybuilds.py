...
def Archive_Tree(sourcefile, destfile, message_header, message1, message2, message3, exclude_dirs, exclude_files):
    ...
    for base, dirs, files in os.walk(sourcefile):
        ...
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.kodinextgentools' in dirs:
                   import time
                   FORCE= '01/01/1980'
...
def Community_Backup():
    ...
    exclude_dirs =  ['plugin.program.kodinextgentools', 'plugin.repository.shadowcrew','cache', 'system', 'Thumbnails', "peripheral_data",'library','keymaps']
