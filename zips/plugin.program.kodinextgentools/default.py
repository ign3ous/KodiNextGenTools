...
AddonID      =  'plugin.program.kodinextgentools'
ADDONS       =  xbmc.translatePath(os.path.join('special://home','addons',''))
ART = os.path.join(ADDONS,AddonID,'resources','art')+os.sep
ADDON        =  xbmcaddon.Addon(id='plugin.program.kodinextgentools')
AddonTitle   =  "Kodi Next Gen Tools"
...
def Categories():
	...
	newbuild = ADDON.getSetting('newbuild')
	guisettings  =  ADDON.getSetting('guisettings')
	skins	=	ADDON.getSetting('skins')
	if mainmenu == 'true':
		extras.addDir('folder','Build Installer','none', 'buildmenu', 'Build_Menu.png','','','')
	if skins == 'true':
		extras.addDir('folder','Skin Installer','none', 'skins', 'skin_Menu.png','','','')
	if newbuild == 'true':
		extras.addDir('folder','New Build Creation Tools','none', 'newbuilds', 'newbuild_Menu.png','','','')
	if guisettings == 'true':
		extras.addDir('folder','Gui Settings Installer','none', 'guisettings', 'Gui_Menu.png','','','')
	if maintenance == 'true':
		extras.addDir('folder','Maintenance Tools','none','tools','Tools.png','','','')
        extras.addDir('folder','Tutorials','none','tutorials','Tools.png','','','')
...
#Tutorials Section
def Tutorials():
    AddLink('Kids Build Install Tutorial', 'https://www.youtube.com/watch?v=dH5IWOpVixo', 'playurl', '.png', isFolder=False, background='')
    setView('movies', 'MAIN')
...
def Wipe_Kodi():
    ...
    if choice == 1:
        if skin!= "skin.confluence":
            dialog.ok('[COLOR=white]Kodi Next Gen Tools[/COLOR]','Please switch to the default Confluence skin','before performing a wipe.','')
            ...
            if choice == 0:
                ...
                exclude_dirs_full =  ['plugin.program.kodinextgentools']
            ...
            choice = xbmcgui.Dialog().yesno("Remove Kodi Next Gen Tools?", 'Do you also want to remove the Kodi Next Gen Tools', 'add-on and have a complete fresh start or would you', 'prefer to keep this on your system?', yeslabel='Remove',nolabel='Keep')
            if choice == 0:
                cache.Remove_Textures()
                ...
                dp.create("[[COLOR=white]Kodi Next Gen Tools[/COLOR][/B]","Checking ",'', 'Please Wait')
...
def Skins_Menu():
	extras.addDir('folder','[COLOR gold]All Skins[/COLOR]','none', 'allskins', '','','','')
	extras.addDir('folder','[COLOR gold]Skins By Author[/COLOR]','none', 'skinsbyauthor', '.png','','','')

	setView('movies', 'MAIN')

def All_Skins_Index():
    	req = urllib2.Request(base+Skin_URL+'All/all.php')
        ...
        for item in feed:
            ...
            if item.has_key('SkinName'):
                sName = item['SkinName']
            if item.has_key('SkinURL'):
                sURL = item['SkinURL']
            if item.has_key('IconURL'):
                iURL = item['IconURL']
            if item.has_key('FanartURL'):
                faURL = item['FanartURL']
            if item.has_key('Description'):
                descr = item['Description']

		addDir(sName,sURL,'skin_wizard',iURL,faURL,descr)
    
	setView('movies', 'MAIN')

def Skins_By_Author():
	extras.addDir('folder','[COLOR gold]Test Author[/COLOR]','none', 'testauthor', '.png','','','')

	setView('movies', 'MAIN')

def Test_Author():
    	req = urllib2.Request(base+Skin_URL+'Test_Author/TestAuthor.php')
        opener = urllib2.build_opener()
        f = opener.open(req)
        feed = json.load(f)
...
def Skin_Wizard():
    ...
    dp = xbmcgui.DialogProgress()
    dp.create("Kodi Next Gen Tools ","Downloading ",'', 'Please Wait')
    ...
    time.sleep(2)
    dp.update(0,"", "Extracting Skin Zip Please Wait")
    print '======================================='
    print addonfolder
    print '======================================='
...
#Builds Section
def BuildMenu():
	extras.addDir('folder','[COLOR gold]All Builds[/COLOR]','none', 'allbuilds', '.png','','','')
	extras.addDir('folder','[COLOR gold]Builds By Category[/COLOR]','none', 'buildsbycategory', '.png','','','')
	extras.addDir('folder','[COLOR gold]Builds By Author[/COLOR]','none', 'buildsbyauthor', '.png','','','')

	setView('movies', 'MAIN')

def ByAuthorMenu():
	extras.addDir('folder','[COLOR gold]Brettus Builds[/COLOR]','none', 'brettusbuilds', 'brettus_Menu.png','','','')
	extras.addDir('folder','[COLOR gold]Spinz Builds[/COLOR]','none', 'spinzbuilds', 'spinz_Menu.png','','','')
...
def ByCategoryMenu():
	...
	extras.addDir('folder','[COLOR gold]Family Builds[/COLOR]','none', 'familybuilds', '.png','','','')
	extras.addDir('folder','[COLOR gold]Kids Builds[/COLOR]','none', 'kidsbuilds', '.png','','','')
	extras.addDir('folder','[COLOR gold]Sports Builds[/COLOR]','none', 'sportsbuilds', '.png','','','')
	if adultbuilds == 'true':
		extras.addDir('folder','[COLOR gold]Adult Builds[/COLOR]','none', 'adultmenu', 'Adult_Menu.png','','','')

	setView('movies', 'MAIN')

def AllBuildsMenu():
	req = urllib2.Request(base+Build_URL+'All/All.php')
	opener = urllib2.build_opener()
	f = opener.open(req)
	feed = json.load(f)
	...
	for item in feed:
		...
		if item.has_key('BuildName'):
			bName = item['BuildName']
		if item.has_key('BuildURL'):
			bURL = item['BuildURL']
		if item.has_key('IconURL'):
			iURL = item['IconURL']
		if item.has_key('FanartURL'):
		...
		addDir(bName,bURL,'wizard',iURL,faURL,descr)

	setView('movies', 'MAIN')

def FamilyMenu():
	req = urllib2.Request(base+Build_URL+'Family/Family.php')
	opener = urllib2.build_opener()
	f = opener.open(req)
	feed = json.load(f)
...
def KidsMenu():
	req = urllib2.Request(base+Build_URL+'Kids/Kids.php')
	opener = urllib2.build_opener()
	f = opener.open(req)
	feed = json.load(f)
...
def SportsMenu():
	req = urllib2.Request(base+Build_URL+'Sports/Sports.php')
	opener = urllib2.build_opener()
	f = opener.open(req)
	feed = json.load(f)
...
def AdultMenu():
	req = urllib2.Request(base+Build_URL+'Adult/Adult.php')
	opener = urllib2.build_opener()
	f = opener.open(req)
	feed = json.load(f)
...
def BrettusMenu():
	req = urllib2.Request(base+Build_URL+'Brettus%20Builds/BrettusBuilds.php')
	opener = urllib2.build_opener()
	f = opener.open(req)
	feed = json.load(f)
...
def SpinzMenu():
	req = urllib2.Request(base+Build_URL+'Spinz%20Builds/SpinzBuilds.php')
	opener = urllib2.build_opener()
	f = opener.open(req)
	feed = json.load(f)
...
#New Build Tools
def NewBuildTools():
	extras.addDir('','[COLOR gold]Clear Cache[/COLOR]','url','clear_cache','Clear_Cache.png','','','')
	extras.addDir('','[COLOR gold]Clear My Cached Artwork[/COLOR]', 'none', 'remove_textures', 'Delete_Cached_Artwork.png','','','')
	extras.addDir('','[COLOR gold]Delete Addon_Data[/COLOR]','url','remove_addon_data','Delete_Addon_Data.png','','','')
	extras.addDir('','[COLOR gold]Delete Old Builds/Zips From Device[/COLOR]','url','remove_build','Delete_Builds.png','','','')
	extras.addDir('','[COLOR gold]Delete Old Crash Logs[/COLOR]','url','remove_crash_logs','Delete_Crash_Logs.png','','','')
	extras.addDir('','[COLOR gold]Delete Packages Folder[/COLOR]','url','remove_packages','Delete_Packages.png','','','')
	extras.addDir('','[COLOR gold]Wipe My Install (Fresh Start)[/COLOR]', 'none', 'wipe_xbmc', 'Fresh_Start.png','','','')
	extras.addDir('','[COLOR gold]Convert Physical Paths To Special[/COLOR]',HOME,'fix_special','Special_Paths.png','','','')
	link = OPEN_URL('http://shadowcrew.info/twins/Community_Builds/templatebuild.txt').replace('\n','').replace('\r','')
	match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
	for name,url,iconimage,fanart,description in match:
		addDir(name,url,'wizard',iconimage,fanart,description)
	setView('movies', 'MAIN')
...
def AddLink(name, url, mode, iconimage, description="", isFolder=True, background=None):
...
elif mode == 'allbuilds'          : AllBuildsMenu()
elif mode == 'buildmenu'		  : BuildMenu()
elif mode == 'buildsbycategory'   : ByCategoryMenu()
elif mode == 'buildsbyauthor'     : ByAuthorMenu()
...
elif mode == 'familybuilds'       : FamilyMenu()
...
elif mode == 'kidsbuilds'         : KidsMenu()
elif mode == 'LocalGUIDialog'     : communitybuilds.Local_GUI_Dialog()
elif mode == 'newbuilds'		  : NewBuildTools()
...
elif mode == 'spinzbuilds'		  : SpinzMenu()
elif mode == 'sportsbuilds'       : SportsMenu()
elif mode == 'skins'			  : Skins_Menu()
elif mode == 'allskins'           : All_Skins_Index()
elif mode == 'skinsbyauthor'      : Skins_By_Author()
elif mode == 'testauthor'         : Test_Author()
elif mode == 'skin_wizard'		  : Skin_Wizard()  
elif mode == 'showinfo'           : communitybuilds.Show_Info(ur)
...
elif mode == 'playurl'            : PlayUrl(name, url, iconimage)
elif mode == 'tutorials'          : Tutorials()
